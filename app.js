...
function generateSpriteSheet(fighterData) {
  ...
  return { width: SPRITE_W, height: SPRITE_H, sheets };
}

function makeFighter(data, x, side) {
  const sprite = generateSpriteSheet(data);
  return {
    ...data,
    side,
    x,
  ...
  manifestEl.textContent = JSON.stringify(
    {
      ...
      player: {
        ...
        spriteAnimations: Object.fromEntries(Object.entries(player.sprite.sheets).map(([name, v]) => [name, v.frames])),
      },
      enemy: {
        name: enemy.name,
        ...
        spriteAnimations: Object.fromEntries(Object.entries(enemy.sprite.sheets).map(([name, v]) => [name, v.frames])),
      },
      wins,
      round,
    },
