...
function generateFighterFromPrompt(prompt, side = "player") {
  ...
  const spritePackKey = lower.includes("sora") && (lower.includes("kingdom hearts") || lower.includes("kh1") || lower.includes("kh" ) )
    ? "sora_kh1"
    : null;

  return {
    name: spritePackKey ? "Sora (Kingdom Hearts 1)" : `${pick(["Nova", "Volt", "Rift", "Astra", "Kairo", "Zeta"], seed)} ${pick(["Striker", "Monk", "Breaker", "Warden", "Specter"], seed >> 1)}`,
    sourcePrompt,
    seed,
    stats,
    style,
    ...
    aiInterpretation: {
      ...
      styleNotes: spritePackKey
        ? "Using builtin high-fidelity Sora sprite pack (KH1-inspired) with full action set."
...
function makeFighter(data, x, side) {
  const sprite = data.spritePackKey && builtinSpritePacks[data.spritePackKey]
    ? builtinSpritePacks[data.spritePackKey]
